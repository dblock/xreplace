...
static const char * strErrorTooManyParenthesis = "Too many ()";
static const char * strErrorUnterminatedParenthesis = "unterminated ()";
static const char * strErrorUnmatchedParenthesis = "unmatched ()";
static const char * strErrorInternalError = "internal error: junk on end";
static const char * strErrorOperand = "*+ operand could be empty";
static const char * strErrorNested = "nested *?+";
static const char * strErrorRange = "invalid [] range";
static const char * strErrorInternalNull = "internal error: \\0|) unexpected";
static const char * strErrorQuestionStar = "?+* follows nothing";
static const char * strErrorStrCspn = "internal error: strcspn 0";
static const char * strErrorNullArg = "NULL argument to regexec";
static const char * strErrorNoRegex = "No regular expression provided yet.";
static const char * strErrorCorrupt = "regexp corruption";
static const char * strErrorCorruptPointers = "corrupted pointers";
static const char * strErrorRegRepeat = "internal error: bad call of RegRepeat";
    ...
    if (flags&REGEXP_SPSTART)
      {
        ...
        for (; scan != NULL; scan = RegNext(scan))
          if (REGEXP_OP(scan) == REGEXP_EXACTLY && strlen(REGEXP_OPERAND(scan)) >= (unsigned int) len)
            {
              longest = REGEXP_OPERAND(scan);
              len = strlen(REGEXP_OPERAND(scan));
            }
        regmust = longest;
        regmlen = (int)len;
      }
...
char *CRegExp::Reg(int paren, int *flagp) {
      ...
      if (regnpar >= REGEXP_NSUBEXP)
		{
          ...
          m_Error = strErrorTooManyParenthesis;
          return NULL;
		}
      parno = regnpar;
      regnpar++;
  ...
  *flagp |= flags&REGEXP_SPSTART;
  ...
  if (paren && *regparse++ != _REGEXP_T(')')) {
    //TRACE0("unterminated ()\n");
    m_Error = strErrorUnterminatedParenthesis;
    return NULL;
  } else if (!paren && *regparse != _REGEXP_T('\0')) {
    if (*regparse == _REGEXP_T(')')) {
      //TRACE0("unmatched ()\n");
      m_Error = strErrorUnmatchedParenthesis;
      return NULL;
    } else {
      //TRACE0("internal error: junk on end\n");
      m_Error = strErrorInternalError;
      return NULL;
    }
    // NOTREACHED
  }  
...
char *CRegExp::RegAtom(int *flagp) {
  ...
  *flagp = REGEXP_WORST;         // Tentatively.
  ...
  default: {
      ...
      regparse--;
      len = strcspn(regparse, REGEXP_META);
      if (len == 0)
		{
          //TRACE0("internal error: strcspn 0\n");
          m_Error = strErrorStrCspn;
          return NULL;
		}
      ender = *(regparse+len);
...
int     CRegExp::RegMatch(char *prog) {
  ...
  for (scan = prog; scan != NULL; scan = next) {
    ...
    case REGEXP_ANYOF:
      if (*reginput == _REGEXP_T('\0') ||
          strchr(REGEXP_OPERAND(scan), *reginput) == NULL)
        return(0);
      reginput++;
      break;
    case REGEXP_ANYBUT:
  ...
  m_Error = strErrorCorruptPointers;
  return(0);
}
...
size_t CRegExp::RegRepeat(char *node) {
  ...
  switch (REGEXP_OP(node))
	...
	case REGEXP_EXACTLY:
      ...
      break;
	case REGEXP_ANYOF:
      return(strspn(reginput, REGEXP_OPERAND(node)));
      break;
	case REGEXP_ANYBUT:
      return(strcspn(reginput, REGEXP_OPERAND(node)));
      break;
	default:                // Oh dear.  Called inappropriately.
      //TRACE0("internal error: bad call of RegRepeat\n");
      m_Error = strErrorRegRepeat;
